...
function DropDownItems({
  ...
  const [items, setItems] =
    useState<RefObject<null | HTMLButtonElement>[]>();
  const [highlightedItem, setHighlightedItem] =
    useState<RefObject<null | HTMLButtonElement>>();

  const registerItem = useCallback(
    (itemRef: RefObject<null | HTMLButtonElement>) => {
  ...
  return (
    <DropDownContext.Provider value={contextValue}>
      <div className="dropdown" ref={dropDownRef as unknown as RefObject<HTMLDivElement>} onKeyDown={handleKeyDown}>
        {children}
      </div>
...
export default function DropDown({
  ...
  const handleOnClick = (e: MouseEvent) => {
    ...
    // isKeyboardInput expects a DOM MouseEvent | PointerEvent, so pass the nativeEvent
    setShouldAutofocus(isKeyboardInput(e.nativeEvent));
  };
