...
import type {JSX, MouseEvent as ReactMouseEvent, CSSProperties} from 'react';
...
export default function ColorPicker({
  ...
  const onBasicColorClick = (
    ...
    setSelfColor(newColor);
    setInputColor(newColor.hex);
    // isKeyboardInput expects a native DOM event; pass React's nativeEvent
    emitOnChange(newColor.hex, isKeyboardInput(e.nativeEvent));
  };
...
function MoveWrapper({className, style, onChange, children}: MoveWrapperProps) {
  ...
  const onMouseDown = (e: ReactMouseEvent<HTMLDivElement>): void => {
    const domEvent = e.nativeEvent as globalThis.MouseEvent;
    ...
    const onMouseMove = (_e: globalThis.MouseEvent): void => {
      draggedRef.current = true;
      skipAddingToHistoryStack = true;
      move(_e);
    };

    const onMouseUp = (_e: globalThis.MouseEvent): void => {
